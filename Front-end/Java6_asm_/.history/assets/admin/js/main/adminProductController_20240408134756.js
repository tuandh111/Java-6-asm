...
app.controller('AdminProductController', function ($scope, $http, $rootScope, $location) {
    ...
    $scope.getSizesByProduct = function (product) {
        let sizes = $scope.infoProductRelative.detailsSizes.filter(detailsSize => detailsSize.product.productId === product.productId).size
        if (sizes.length > 0) {
            console.log("sizes by product", sizes);
        } else {
            console.log("ppp");
        }
    }
