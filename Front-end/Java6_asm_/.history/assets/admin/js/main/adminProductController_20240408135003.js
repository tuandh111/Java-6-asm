...
app.controller('AdminProductController', function ($scope, $http, $rootScope, $location) {
    ...
    $scope.getSizesByProduct = function (product) {
        let detailsSizeByProduct = $scope.infoProductRelative.detailsSizes.filter(detailsSize => detailsSize.product.productId === product.productId)[0]
        if (detailsSizeByProduct.length > 0) {
            console.log("detailsSizeByProduct by product", detailsSizeByProduct);
        } else {
            console.log("ppp");
        }
    }
