...
app.controller('AdminProductController', function ($scope, $http, $rootScope, $location) {
    ...
    $scope.initPage = function () {
        console.log("$scope.infoProductRelative.discounts", $scope.infoProductRelative.discounts);
        // Thực hiện các tác vụ khởi tạo ở đây
        // $scope.infoProductRelative.products.forEach(function (product) {
        //     product.imageProd = $scope.getImageByProduct(product);
        //     product.discountProd = $scope.getDiscountByProduct(product);
        // });
    };
    // ng-init="imageProd=getImageByProduct(product);
    // discountProd=getDiscountByProduct(product)"
    $scope.listImg();
    $scope.listInfo();
    $scope.initPage();
})
