...
app.controller('AdminBrandController', function ($scope, $http, $rootScope, $location) {
    ...
    $rootScope.listBrandsShow = [];//để chứa kết quả show lên giao diện
    ...
    $scope.getAllBrand = function () {
        $http.get(url + "/api/v1/auth/twobee/brands-and-count-product").then(
            response => {
                ...
                $rootScope.listBrandsShow = $scope.listBrandsFilter
                console.log("$rootScope.listBrandsShow ", $rootScope.listBrandsShow);
                $scope.pageCount = Math.ceil($rootScope.listBrandsShow.length / $scope.pageSize);
    ...
    $scope.filterBrand = function () {
        $scope.filterByBrandsName = function () {
            ...
            $rootScope.listBrandsShow = $scope.filteredBrands;
            $scope.firtPage();
        }
    ...
    $scope.crudBrand = function () {
        ...
        //     $rootScope.listBrandsShow.push(data);
        // }
        $scope.createBrand = function () {
            ...
            $http.post(url + "/api/v1/auth/twobee/brands", requsetBrandJSON).then(
                respone => {
                    ...
                    Swal.fire({
                        title: "Thành công!",
                        text: "Đã thêm danh mục " + respone.data.nameBrand + " thành công!",
                        icon: "success"
                    });
                    $rootScope.listBrandsShow.push(respone.data);
                    $scope.resetForm();
