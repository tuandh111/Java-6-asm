...
app.controller('AdminBrandController', function ($scope, $http, $rootScope, $location) {
    ...
    $scope.crudBrand = function () {
        ...
        $scope.pushData = function (data) {
            $scope.listBrandsShow.push(data);
        }
        $scope.createBrand = function () {
            var requsetBrand = angular.copy($scope.formBrand)
            var requsetBrandJSON = angular.toJson(requsetBrand)
            $http.post(url + "/api/v1/auth/twobee/brands", requsetBrandJSON).then(
                respone => {
                    ...
                    $scope.pushData(data);
                    // $scope.listBrandsShow.push(data);//chưa làm mới được danh sách
                    $scope.resetForm();
                    ...
                    //$scope.$apply(function () { $scope.listBrandsShow.push(data); })
                }
