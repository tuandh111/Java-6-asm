...
app.controller('AdminProductController', function ($scope, $http, $rootScope, $location) {
    ...
    // }
    $scope.urlImg = (filename) => {
        var token = authService.getToken(); // Lấy token từ service xác thực

        return $http.get(url + "/management/twobee/" + "images" + "/" + filename, {
            responseType: 'arraybuffer', // Chỉ định kiểu dữ liệu trả về là array buffer
            headers: {
            ...
            var blob = new Blob([response.data], { type: imageType });
