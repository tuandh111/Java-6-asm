    ...
    $scope.getSizesByProduct = function (product) {
        ...
        if (detailsSizeByProduct.length > 0) {
            detailsSizeByProduct.forEach((detail, index) => {
                //console.log("detailsSizeByProduct by product " + product.productId, detail.size.sizeName);
                sizesString += detail.size.sizeName; // Thêm sizeName vào chuỗi
                if (index < detailsSizeByProduct.length - 1) {
                    sizesString += ", "; // Thêm dấu phẩy và khoảng trắng nếu không phải là phần tử cuối cùng
                }
            })
        }
        return sizesString;
    ...
    $scope.getColorsByProduct = function (product) {
        ...
        if (detailsColorByProduct.length > 0) {
            detailsColorByProduct.forEach((detail, index) => {
                //console.log("detailsSizeByProduct by product " + product.productId, detail.size.sizeName);
                colorsString += detail.color.colorName; // Thêm sizeName vào chuỗi
                if (index < detailsColorByProduct.length - 1) {
                    colorsString += ", "; // Thêm dấu phẩy và khoảng trắng nếu không phải là phần tử cuối cùng
                }
            })
        }
        return colorsString;
