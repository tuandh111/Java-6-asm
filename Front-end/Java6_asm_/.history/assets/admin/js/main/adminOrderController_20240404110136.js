...
app.controller('AdminOrderController', function ($scope, $http, $rootScope, $location) {
    ...
    $scope.filterOrder = function () {
        ...
        // $scope.getProdById = function (prodId) {
        //     $rootScope.prodInfo=$scope.listProductsFilter.filter(prod =>prod.product.id==prodId)
        // }
        $scope.getProdById = function (prodId) {
            return $scope.listProductsFilter.filter(prod => prod.product.id == prodId);
        };
