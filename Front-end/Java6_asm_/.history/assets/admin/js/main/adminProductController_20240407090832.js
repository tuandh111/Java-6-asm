...
app.controller('AdminProductController', function ($scope, $http, $rootScope, $location) {
    ...
    $scope.getImageByProduct = function (product) {
        ...
        // console.log("$scope.infoProductRelative.productImage", $scope.infoProductRelative.productImage);
        $scope.listProductImg = $scope.infoProductRelative.productImage.filter(prodImg => prodImg.product.productId === product.productId)[0].imageName;
        console.log($scope.listProductImg);
        return $scope.listProductImg
    };
