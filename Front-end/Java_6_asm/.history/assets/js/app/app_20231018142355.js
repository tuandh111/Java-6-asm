...
function ForgetController($scope, $http) {
    ...
    $http({
        ...
        $scope.sendEmail = function () {
            console.log($scope.formForgetPass.forgetPassInput.$valid);
            if (formForgetPass.forgetPassInput.$valid) {
                ...
                $scope.listUsers.forEach(element => {
                    if (element.email == emailInput) {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Reset Failed',
                            text: "Email không đúng. Bạn còn " + $scope.timesInputEmail + " lần nhập!"
                        });
                    }
                })
